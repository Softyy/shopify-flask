...
def request_access():
    ...
    with open('.allowed_shops','r+') as f:
        for shop_info in f:
            allowed_shops.append({ key:value for key,value in [info.split('=') for info in shop_info.split(',')]})
...
def template_test2():
    ...
    with open('.requested_shops','r') as f:
        for shop_info in f:
            info = { key:value for key,value in [info.split('=') for info in shop_info.split(',')]}
    ...
    with open('.requested_shops','w') as f:
        for info in request_shops:
            f.write(f'shop={info["shop"]},state={info["state"]},timestamp={info["timestamp"]}\n')
